...
fn main() {
  ...
  if IS_DEV_ENV {
    ...
    // TODO. Could this be chosen automatically by HAMGRAPH ? Or is it not a good thing ?
    hg::set_install_path(dirs::home_dir()
    .unwrap_or_else(|| { hamgraph::errors::prompt_err_and_panic("no home dir!", "", None); })
    .join("/usr/local/example0")); // TODO (improve on windows/linux etc) (distinguish install path / game path)
  }
